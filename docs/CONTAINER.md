...
- Rendering uses the headless Python renderer (PyVista with matplotlib fallback). Adjust the Dockerfile if you add external visualization tooling later.
...
When dependencies change (new Debian release, new rendering stack, etc.):
